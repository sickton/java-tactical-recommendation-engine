...
public class TacticalRulesTest {
    ...
    @Test
    public void testEarlyTeam() {
       MatchContext etw = new MatchContext("LIV-CITY", homeTeam, awayTeam, 1, 0, 17, 51.0, 49.0);
       EarlyTeamWinning e = new EarlyTeamWinning();
       assertEquals(Tactic.MID_BLOCK, e.recommend(etw, homeTeam));
       assertEquals(Tactic.MID_BLOCK, e.recommend(etw, awayTeam));

       MatchContext etd = new MatchContext("LIV-CITY", homeTeam, awayTeam, 2, 2, 10, 48, 52);
       EarlyTeamDrawing ed = new EarlyTeamDrawing();
       assertEquals(Tactic.MID_BLOCK, ed.recommend(etd, homeTeam));
       assertEquals(Tactic.MID_BLOCK, ed.recommend(etd, awayTeam));

        MatchContext etl = new MatchContext("LIV-CITY", homeTeam, awayTeam, 0, 2, 15, 48, 52);
        EarlyTeamLosing el = new EarlyTeamLosing();
        assertEquals(Tactic.POSSESSION, el.recommend(etl, homeTeam));
        assertEquals(Tactic.MID_BLOCK, el.recommend(etl, awayTeam));
    }

    @Test
    public void testMiddleTeam() {
        MatchContext mtw = new MatchContext("LIV-CITY", homeTeam, awayTeam, 2, 0, 40, 53, 47);
        MiddleTeamWinning mw = new MiddleTeamWinning();
        assertEquals(Tactic.MID_BLOCK, mw.recommend(mtw, homeTeam));
        assertEquals(Tactic.MID_BLOCK, mw.recommend(mtw, awayTeam));

        MatchContext mtd = new MatchContext("LIV-CITY", homeTeam, awayTeam, 0, 0, 52, 49, 51);
        MiddleTeamDrawing md = new MiddleTeamDrawing();
        assertEquals(Tactic.MID_BLOCK, md.recommend(mtd, homeTeam));
        assertEquals(Tactic.MID_BLOCK, md.recommend(mtd, awayTeam));

        MatchContext mtl = new MatchContext("LIV-CITY", homeTeam, awayTeam, 0, 2, 60, 50, 50);
        MiddleTeamLosing ml = new MiddleTeamLosing();
        assertEquals(Tactic.HIGH_PRESS, ml.recommend(mtl, homeTeam));
        assertEquals(Tactic.MID_BLOCK, ml.recommend(mtl, awayTeam));
    }

    @Test
    public void testLateTeam() {
        MatchContext ltw = new MatchContext("LIV-CITY", homeTeam, awayTeam, 1, 0, 85, 52, 48);
        LateTeamWinning lw = new LateTeamWinning();
        assertEquals(Tactic.POSSESSION,  lw.recommend(ltw, homeTeam));
        assertEquals(Tactic.MID_BLOCK, lw.recommend(ltw, awayTeam));

        MatchContext ltd = new MatchContext("LIV-CITY", homeTeam, awayTeam, 0, 0, 82, 40, 60);
        LateTeamDrawing ld = new LateTeamDrawing();
        assertEquals(Tactic.MID_BLOCK, ld.recommend(ltd, homeTeam));
        assertEquals(Tactic.POSSESSION, ld.recommend(ltd, awayTeam));

        MatchContext ltl = new MatchContext("LIV-CITY", homeTeam, awayTeam, 0, 3, 80, 32, 68);
        LateTeamLosing lt = new LateTeamLosing();
        assertEquals(Tactic.HIGH_PRESS, lt.recommend(ltl, homeTeam));
        assertEquals(Tactic.HIGH_PRESS, lt.recommend(ltl, awayTeam));
    }
}
