...
public class LateGameTactics extends TacticalRule {
    ...
    public TacticRecommendation recommendWithConfidence(MatchContext context, Team team, Map<TacticKey, TacticSuggestion> tacticMap) {
        ...
        if (isTeamWinning(context, team)) {
            ...
            dAttack += ADJUST_THREE; dControl += ADJUST_TWO; dDefence -= ADJUST_ONE;
        } else if (isTeamLosing(context, team)) {
        ...
        TacticKey key2 = new TacticKey(team.getSquad().getTeamStyle(), adjustWeights(ca, cc, cd), GamePhase.LATE_GAME);
